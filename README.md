...
<body>
    ...
    <div id="rotate-screen">
        ...
        <h2>VIRE A TELA</h2>
        <p>Gire para o modo paisagem para jogar!</p>
    </div>
    <div id="ui-layer">
...
<script>
    ...
    function selectEmoji(el, val) {
        ...
        selectedFeedbackEmoji = val;
    }
    function enviarFeedback() {
        playClick();
        showFeedbackMessage("Obrigado pelo seu feeedback!");
        document.getElementById("feedback-section").style.display = "none";
        document.getElementById("feedback-text").value = "";
    }
    function showFeedbackMessage(msg) {
        achievementToast.innerText = msg;
        achievementToast.style.display = "block";
        setTimeout(() => { achievementToast.style.display = "none"; }, 3000);
    }
    function playMorte() {
    ...
    // =====================================================
    // SISTEMA DE JOYSTICK E PULO CORRIGIDO
    // Usa pointerId separado para cada controle,
    // permitindo uso simultâneo sem conflito.
    // =====================================================
    let joystick = {
        active: false,
        pointerId: null,
    ...
    // --- BOTÃO DE PULO: captura o ponteiro exclusivamente no botão ---
    ...
    // --- JOYSTICK: captura o ponteiro exclusivamente na área do joystick ---
    joyArea.addEventListener("pointerdown", (e) => {
        e.preventDefault();
        e.stopPropagation();
        // Só aceita um toque por vez no joystick
    ...
    joyArea.addEventListener("pointercancel", (e) => {
        if (e.pointerId !== joystick.pointerId) return;
        joystick.active = false;
        joystick.pointerId = null;
        joystick.x = 0;
    ...
    function drawMapTexture(c, p) {
        ...
        for(let i = 0; i <= p.w; i += 25) {
            c.beginPath();
            c.arc(p.x + i, p.y + 25, 15, 0, Math.PI);
            c.fill();
        }
        ctx.restore();
    }
